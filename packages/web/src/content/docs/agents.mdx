...
OpenCode comes with three built-in primary agents, **Build**, **Plan**, and **Learn**. We'll
look at these below.
...
OpenCode comes with three built-in primary agents and three built-in subagents.
...
A read-only agent for learning a codebase. It can read, search, and explain code and answer your questions, but all `file edits` are set to `deny`, so it never modifies your files.

This agent is useful when you want to understand how something works before changing it.
